...
| | |
|---|---|
| **Unit** | KKN-PPM UGM 2025-YO002, Sub Unit 1, Puskesmas Karanganom, Karanganom, Klaten |
...
CERIA adalah sistem pendataan dan skrining kesehatan digital untuk Puskesmas Remaja & Lansia di Puskesmas Karanganom. Sistem menggantikan formulir kertas "Catatan Hasil Pemeriksaan Kesehatan" dengan aplikasi web yang otomatis mengklasifikasikan hasil pemeriksaan (IMT, tensi, gula darah, kolesterol, lingkar pinggang) sesuai standar WHO/Kemenkes, lalu menyimpannya secara terpusat dan tersinkronisasi untuk seluruh kader puskesmas.
...
|---|---|
| Pencatatan hasil pemeriksaan puskesmas masih manual di atas formulir kertas | Data sulit direkap, mudah hilang/rusak, dan tidak ada riwayat per-warga antar sesi |
...
| Staf Tenaga Kesehatan sebagian berusia lanjut, terbatas literasi digital | Sistem yang rumit berisiko tidak terpakai setelah mahasiswa KKN selesai |
...
**Tujuan Produk**
1. Mempercepat dan menstandarkan proses pencatatan hasil pemeriksaan kesehatan di puskesmas.
...
4. Memastikan sistem dapat terus dipakai dan dikelola oleh puskesmas/puskesmas tanpa bergantung pada mahasiswa KKN setelah program selesai.
...
|---|---|
| **Staf Tenaga Kesehatan** (pengguna utama, mayoritas usia lanjut) | Input cepat, langkah sederhana, tidak butuh pelatihan IT mendalam |
...
| **Privasi data** | Data NIK & kesehatan tunduk pada UU PDP — perlu lembar persetujuan (consent) saat pendataan warga, data hanya dipakai untuk keperluan puskesmas/Puskesmas |
...
| Backup data | Ekspor CSV manual oleh kader setiap selesai sesi puskesmas (bukan layanan backup berbayar) | Tidak terbatas, manual | ✅, menggantikan fitur backup otomatis yang berbayar di tier gratis Supabase |
| Desain/Ikon | SVG/CSS buatan sendiri, tanpa Canva Pro/asset berbayar | — | ✅ |
| Koneksi internet di puskesmas | WiFi balai desa/puskesmas yang sudah ada, atau hotspot HP kader (operasional KKN, bukan biaya produk) | — | Diluar lingkup biaya produk |
...
> **Catatan jujur:** kalau suatu saat skala data jauh melebihi satu desa (misal diadopsi multi-kecamatan), tier gratis ini akan butuh upgrade berbayar. Untuk skala satu Puskesmas Karanganom, ini tidak akan terjadi dalam waktu dekat.
...
```
Staf Puskesmas (HP/Laptop, dimana saja)
...
1. Seluruh akun (Vercel, Supabase, GitHub) didaftarkan dengan **email institusional** (puskesmas/desa), bukan email pribadi mahasiswa — sejak awal, bukan ditransfer belakangan.
...
| Tidak ada backup otomatis di tier gratis | Wajibkan ekspor CSV manual setiap akhir sesi puskesmas |
...
| 5–6 | Rollout penuh ke seluruh kader puskesmas, pendataan warga berjalan |
| 7 | Pelatihan admin lokal, penyusunan manual handover |
| 8 | Serah terima resmi ke puskesmas/desa, evaluasi akhir |
